...
ShareHandleWithDriver(_In_ HANDLE hDevice)
{
    DWORD dwNbBytesReturned;


    HANDLE hDataEvent = ::CreateEvent(NULL, TRUE, FALSE, NULL);
...
--*/
static DWORD
FetchNextIrpFromDevice(_In_ HANDLE hDevice, _In_ HANDLE hEvent, _In_ Session& Session)
{
...
--*/
DWORD
FetchAllIrpFromDevice(
    _In_ HANDLE hDevice,
    _In_ HANDLE hEvent,
    _In_ Session& Session,
    _Out_ PDWORD lpdwNumberOfIrpDumped)
{
...
--*/
DWORD
IrpCollectorThreadRoutine(_In_ LPVOID lpParameter)
{
    ...
    //
    SafeHandle hIrpDumperDevice(::CreateFile(
    ...
    //
    SafeHandle hIrpDataEvent(ShareHandleWithDriver(hIrpDumperDevice.Get()));
    ...
    const HANDLE Handles[2] = {Sess.m_hTerminationEvent, hIrpDataEvent.Get()};
...
--*/
static Task
SendTaskToDriver(_In_ Task task, _In_ HANDLE hDevice)
{
...
--*/
DWORD
BackendConnectionHandlingThread(_In_ LPVOID lpParameter)
{
    Session& Sess = *(reinterpret_cast<Session*>(lpParameter));
    ...
    const HANDLE Handles[2] = {Sess.m_hTerminationEvent, Sess.RequestTasks.m_hPushEvent};
...
_Success_(return )
BOOL
StartBackendManagerThread(_In_ PVOID lpParameter)
{
    HANDLE hThread;
    DWORD dwThreadId;
    Session& Sess = *(reinterpret_cast<Session*>(lpParameter));
    ...
    //
    hThread = ::CreateThread(NULL, 0, BackendConnectionHandlingThread, lpParameter, CREATE_SUSPENDED, &dwThreadId);
    ...
    //
    hThread = ::CreateThread(NULL, 0, IrpCollectorThreadRoutine, lpParameter, CREATE_SUSPENDED, &dwThreadId);
